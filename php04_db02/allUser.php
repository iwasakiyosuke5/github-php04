...
if ($search !== '') {
  ...
  $sql .= " WHERE id LIKE :search OR name LIKE :search OR lid LIKE :search";
}
  ...
  <nav class="navbar navbar-default">
    <div class="container-fluid">
      <div class="w-4/5 flex justify-between navbar-header">
        ...
        <div>ログイン中:[<?=$_SESSION["name"]?>さん]<a class="ml-2 navbar-brand border-b-2 border-gray-400" href="logout.php">ログアウト</a></div>
      </div>
      <div>
        <nav class="text-red-400 navbar navbar-default">ユーザー一覧</nav>
        <a class="navbar-brand bg-red-300 rounded px-1" href="index.php">データ登録へ</a>
        <a class="navbar-brand bg-red-300 rounded px-1" href="select.php">論文一覧へ</a>  
      </div>
    </div>
  </nav>
</header>
        ...
        <form action="userDelete.php" method="post" onsubmit="showSpinner()">
          <table class="text-center text-xl w-full">
            <tr class="">
              ...
              <th class="border-r border-gray-100 px-2">name</th>
              <th class="border-r border-gray-100 px-2">lid</th>
            ...
            <?php $count = 0; foreach($values as $v){ $count++;?>
              <!-- 「$v){」を「$v):」 とした場合、-->
                ...
                <td style=""><?php if($v["id"] !== 1):?><input type="checkbox" name="delete_ids[]" value="<?=$v["id"]?>"><?php endif;?></td>
              </tr>
